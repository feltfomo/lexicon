...
func TestBuildLineViolation(t *testing.T) {
	cases := map[string]bool{
		`building '/nix/store/aaaa1111bbbb2222cccc3333dddd4444-lexicon-install.drv'...`: true,
		`  /nix/store/eeee5555ffff6666gggg7777hhhh8888-nixos-system-furnish-vm.drv`:     true,
		`these 3 derivations will be built:`:                                            true,
		`copying path '/nix/store/xxxx-etc' from 'https://cache.nixos.org'...`:          false,
		`/nix/store/zzzz-nixos-system-furnish-vm-26.05`:                                 false,
	}
...
func TestNormalizePubKey(t *testing.T) {
	name, key, ok := NormalizePubKey("  lexicon-1:abc123==  ")
	if !ok || name != "lexicon-1" || key != "abc123==" {
