...
// stage record is the recorded outcome of one state-machine stage.
...
// proof records an expected value beside what a stage observed.
type Proof struct {
	Name     string `json:"name"`
...
// artifact records a frozen golden file with its path, content hash, and mode.
...
// manifest is the machine-readable record of one rebuild run and is rendered
// as json plus a human-readable text report.
...
// write json writes the manifest as indented json.
...
// write human renders a readable summary.
