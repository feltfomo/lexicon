	...
	for _, ln := range lines {
		...
		if watch != nil {
			if err := watch(ln); err != nil {
				// fake mirrors stdout into combined while the exec runner tests cover
				// the real stream split.
...
// these constants serve the signed cache closure proof for both explicit
// roots. nix derivation show yields each output path, then nix path-info against
// the cache must return a complete recursive closure signed by the revision key.
const (
	gateDrv      = "/nix/store/dddd0000dddd0000dddd0000dddd0000-nixos-system-furnish-vm.drv"
	gateOutPath  = "/nix/store/qr05v34zqr05v34zqr05v34zqr05v34z-nixos-system-furnish-vm-26.05"
...
// the production gate must reach the merged copy and build half after both
// static root proofs pass, preserving the non-vacuous fetched zero paths check.
...
// path-info exits nonzero when a referenced path is missing from the signed
// cache, so the gate must fail closed.
...
// a closure path signed only by an untrusted golden key must fail closed.
func TestBuildPlanGateFailsUntrustedSignature(t *testing.T) {
	...
	body := fmt.Sprintf(`[{"path":%q,"signatures":[%q]},{"path":%q,"signatures":[%q]},{"path":%q,"signatures":[%q]}]`,
		gateOutPath, key+":sigA==", gateDiskoOut, key+":sigB==", gateDep, "lexicon-furnish-golden-cafebabe:sigX==")
	h.runner = gateRunner(body, 0)
	h.manifest.DrvPaths = map[string]string{"toplevel": gateDrv, "disko": gateDiskoDrv}
	if err := h.stageStaticBuildPlanGate(context.Background()); err == nil {
...
// the full pipeline must place destructive stages behind the confirm gate.
func TestAllStagesOrderingConfirmBeforeProvision(t *testing.T) {
	...
	// a fresh all run must build and sign the cache before validation, and the
	// gate needs the signing key public half before serving it to the guest.
...
func TestEvalDrvStoresCleanStdoutPath(t *testing.T) {
	want := "/nix/store/aaa1111bbbb2222cccc3333dddd4444eeee5555-nixos-system-furnish-vm-26.05.drv"
	...
	if len(evalAttrs) != 2 || !strings.HasSuffix(evalAttrs[1], "#nixosConfigurations.furnish-vm.config.system.build.destroyFormatMount.drvPath") {
		t.Fatalf("disko eval did not select the pinned destroy-format-mount derivation: %v", evalAttrs)
	}
}

// a multiline captured value with stderr warnings before the drv must fail
// closed rather than poisoning export-sign.
func TestEvalDrvRejectsPoisonedOutput(t *testing.T) {
	runner := fakeRunner{responder: func(spec CmdSpec) ([]string, int) {
		return []string{
			"warning: fetching git input",
			"Using saved setting for 'extra-substituters'",
			"/nix/store/aaa1111bbbb2222cccc3333dddd4444eeee5555-nixos-system-furnish-vm-26.05.drv",
		}, 0
	}}
	h := testHarness(t, runner)
	h.preparedSource = t.TempDir()
...
// exec runner must expose stdout separately from the combined view.
...
// the watcher must still scan stderr so a build line trips the emergency stop.
...
// scanDiskoDevices extracts whole disk targets from the disko script and
// ignores partition paths while deduplicating repeated devices.
...
// a second whole disk must be surfaced so the exact device check fails closed.
...
func TestSubstitutionCompletenessCommandsAndSuccessCleanup(t *testing.T) {
	...
	h.runner = fakeRunner{responder: func(spec CmdSpec) ([]string, int) {
		...
		if slicesContain(spec.Args, "--all") {
			...
			return []string{fmt.Sprintf(`[{"path":%q},{"path":%q},{"path":%q%c]`, gateOutPath, gateDiskoOut, gateDep, 125)}, 0
		}
		if len(spec.Args) > 0 && spec.Args[0] == "path-info" {
...
func TestReusePreparedSourcePinsCacheInput(t *testing.T) {
	...
	for _, rel := range transportFixturePaths {
		fixture := filepath.Join(source, rel)
		if err := os.MkdirAll(filepath.Dir(fixture), 0o755); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(fixture, []byte("transport key fixture\n"), 0o644); err != nil {
			t.Fatal(err)
		}
		runGit("add", rel)
	}
	...
	if h.manifest.PreparedSourceHash == "" {
