...
func TestManifestEmitsObservedMeasurements(t *testing.T) {
	...
	for _, want := range []string{"\"prepared_source_reused\":true", "\"prepared_source_origin_run\":\"/evidence/build-cache\"", "\"gate_build_count\":0", "\"gate_fetch_count\":9", "\"provision_build_count\":0", "\"overlay_proof\":true", "\"base_untouched\":true"} {
		if !strings.Contains(string(b), want) {
			t.Fatalf("json missing %s: %s", want, b)
		}
	}
	h := m.Human()
	for _, want := range []string{"prepared reused: true", "prepared origin run: /evidence/build-cache", "gate build count: 0", "gate fetch count: 9", "provision build count: 0"} {
