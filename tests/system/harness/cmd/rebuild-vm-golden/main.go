...
import (
	...
	"github.com/feltfomo/lexicon/tests/system/harness/internal/harness"
)
...
func run() int {
	...
	var (
		...
		host             = fs.String("host", "furnish-vm", "nixos host to rebuild")
...
func defaultStateDir() string {
	if xdg := os.Getenv("XDG_STATE_HOME"); xdg != "" {
		return filepath.Join(xdg, "lexicon-furnish-vm", "rebuild-vm-golden")
	}
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return filepath.Join(os.TempDir(), "lexicon-furnish-vm", "rebuild-vm-golden")
	}
	return filepath.Join(home, ".local", "state", "lexicon-furnish-vm", "rebuild-vm-golden")
}
